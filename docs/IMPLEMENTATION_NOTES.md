...
- **Filtres avancés opt-in** : qualité de sortie RSI, tendance EMA/RSI HTF et régime ATR rapide/lent utilisent exclusivement des bougies clôturées, des handles persistants et un comportement fail-closed.
...
- **Compilation MQL5** : L'environnement Linux d'exécution ne disposant pas du compilateur GUI propriétaire MetaEditor MT5, la compilation binaire `.ex5` doit être effectuée directement dans MetaTrader 5 par l'utilisateur ou le système de backtest.
