...
enum ENUM_EXIT_GEOMETRY_MODE
{
   EXIT_GEOMETRY_LEGACY = 0,     // Existing Fibonacci or optional fixed-R target
   EXIT_GEOMETRY_STRUCTURE = 1   // Closed-bar structure stop and P1 liquidity target
};
...
//--- Market-driven exit geometry
input group "=== Market-Driven Exit Geometry ==="
input ENUM_EXIT_GEOMETRY_MODE InpExitGeometryMode = EXIT_GEOMETRY_LEGACY; // Legacy or structure-derived exits
input int      InpStructuralStopLookbackBars = 1;      // Closed bars used for invalidation extreme (1 = anchor only)
input double   InpStructuralStopBufferATR = 0.30;      // ATR buffer beyond closed-bar extreme
input double   InpStructuralMinRiskATR = 0.25;         // Minimum Entry-to-SL distance in ATR units
...
double      m_last_sizing_actual_loss = 0.0;
double      m_last_sizing_actual_risk_pct = 0.0;
double      m_last_sizing_required_margin = 0.0;
double      m_last_sizing_margin_limit = 0.0;
bool        m_last_sizing_margin_capped = false;
...
string FunnelEventName(const ENUM_FUNNEL_EVENT event)
{
   switch (event)
   {
      ...
      case FUNNEL_REJECT_NET_REWARD_RISK: return "REJECT_NET_REWARD_RISK";
      ...
      case FUNNEL_SIZE_MARGIN_CAPPED: return "SIZE_MARGIN_CAPPED";
...
int OnInit()
{
   ...
   if (InpMinRangeATR > 0.0 || InpMaxRangeATR > 0.0 || InpUseAdaptiveSL ||
       ...
       InpExitGeometryMode == EXIT_GEOMETRY_STRUCTURE)
   {
...
void ProcessStateWaitingForAnchor()
{
   ...
   // Exit geometry is selected from information already closed at setup time.
   // Structural mode never solves for a requested R multiple: the stop comes
   // from invalidation structure and the target from the known P1 liquidity.
   bool exit_geometry_ok = (InpExitGeometryMode == EXIT_GEOMETRY_STRUCTURE)
                           ? ApplyStructuralExitGeometry()
                           : AdaptSLTPToVolatility();
   if (!exit_geometry_ok)
   {
      RecordFunnel(FUNNEL_REJECT_ADAPTIVE_GEOMETRY);
      if (InpVerboseLog)
         Print("WARNING: [RSIFibEA] Exit geometry adjustment failed or became invalid. Setup aborted.");
      ResetSetupToIdle();
      return;
   }

   // RR is an observation and optional quality gate, never a target solver.
   if (!EvaluateSetupRewardRisk(true, true))
   {
      RecordFunnel(FUNNEL_REJECT_NET_REWARD_RISK);
      ResetSetupToIdle();
      return;
   }
   ...
   if (!EvaluateSetupRewardRisk(false, true))
   {
      RecordFunnel(FUNNEL_REJECT_NET_REWARD_RISK);
      Print("GUARD REJECT: Natural net reward/risk deteriorated before sizing. Setup aborted.");
      ResetSetupToIdle();
      return;
   }
...
void ProcessStatePendingOrder(bool is_new_bar)
{
       ...
       !EvaluateSetupRewardRisk(false, true))
   {
      ...
      Print("INFO: [RSIFibEA] Pending order no longer satisfies risk/spread/session/contract/net-RR guards. Cancelling.");
      CancelPendingOrder();
      return;
   }
...
string ExitGeometryModeName()
{
   return (InpExitGeometryMode == EXIT_GEOMETRY_STRUCTURE)
          ? "STRUCTURE" : "LEGACY";
}

// Build both exits from data that is fully known when the anchor closes.
// The stop sits beyond the recent closed-bar invalidation extreme plus an ATR
// buffer. The target is the already-observed P1 liquidity level, optionally
// offset toward entry by broker ticks to avoid demanding a break through it.
bool ApplyStructuralExitGeometry()
{
   if (m_setup.dir != SIGNAL_BUY && m_setup.dir != SIGNAL_SELL)
      return false;
   if (m_atr_handle == INVALID_HANDLE)
   {
      Print("ERROR: [RSIFibEA] Structural exit geometry requires a valid ATR handle.");
      return false;
   }
   ...
   if (!GetATR(1, atr))
   {
      Print("WARNING: [RSIFibEA] Cannot read the closed-bar ATR for structural exits.");
      return false;
   }
   ...
   int lookback = MathMax(1, InpStructuralStopLookbackBars);
   double structural_extreme = (m_setup.dir == SIGNAL_BUY)
                               ? iLow(_Symbol, m_timeframe, 1)
                               : iHigh(_Symbol, m_timeframe, 1);
   if (structural_extreme <= 0.0 || !MathIsValidNumber(structural_extreme))
      return false;

   for (int shift = 2; shift <= lookback; shift++)
   {
      double candidate = (m_setup.dir == SIGNAL_BUY)
                         ? iLow(_Symbol, m_timeframe, shift)
                         : iHigh(_Symbol, m_timeframe, shift);
      if (candidate <= 0.0 || !MathIsValidNumber(candidate))
         return false;
      if (m_setup.dir == SIGNAL_BUY && candidate < structural_extreme)
         structural_extreme = candidate;
      if (m_setup.dir == SIGNAL_SELL && candidate > structural_extreme)
         structural_extreme = candidate;
   }

   double stop_buffer = InpStructuralStopBufferATR * atr;
   double target_buffer = (double)InpStructuralTargetBufferTicks * tick_size;
   double min_risk_distance = InpStructuralMinRiskATR * atr;
   if (!MathIsValidNumber(stop_buffer) || stop_buffer < 0.0 ||
       !MathIsValidNumber(min_risk_distance) || min_risk_distance <= 0.0 ||
       !MathIsValidNumber(target_buffer) || target_buffer < 0.0)
      return false;

   if (m_setup.dir == SIGNAL_BUY)
   {
      double raw_structural_stop = structural_extreme - stop_buffer;
      double raw_risk_floor_stop = m_setup.entry_price - min_risk_distance;
      m_setup.stop_price = NormalizePriceDirectional(
         MathMin(raw_structural_stop, raw_risk_floor_stop), -1);
      m_setup.target_price = NormalizePriceDirectional(
         m_setup.P1 - target_buffer, -1);
      m_setup.visual_target_price = NormalizePriceDirectional(m_setup.P1, 0);
   }
   else
   {
      double raw_structural_stop = structural_extreme + stop_buffer;
      double raw_risk_floor_stop = m_setup.entry_price + min_risk_distance;
      m_setup.stop_price = NormalizePriceDirectional(
         MathMax(raw_structural_stop, raw_risk_floor_stop), 1);
      m_setup.target_price = NormalizePriceDirectional(
         m_setup.P1 + target_buffer, 1);
      m_setup.visual_target_price = NormalizePriceDirectional(m_setup.P1, 0);
   }
   m_setup.partial_tp_price = 0.0;

   bool valid = (m_setup.stop_price > 0.0 && m_setup.target_price > 0.0 &&
                 m_setup.visual_target_price > 0.0 &&
                 MathIsValidNumber(m_setup.stop_price) &&
                 MathIsValidNumber(m_setup.target_price) &&
                 MathIsValidNumber(m_setup.visual_target_price));
   if (valid && m_setup.dir == SIGNAL_BUY)
      valid = (m_setup.stop_price < m_setup.entry_price &&
               m_setup.entry_price < m_setup.target_price &&
               m_setup.target_price <= m_setup.visual_target_price);
   if (valid && m_setup.dir == SIGNAL_SELL)
      valid = (m_setup.visual_target_price <= m_setup.target_price &&
               m_setup.target_price < m_setup.entry_price &&
               m_setup.entry_price < m_setup.stop_price);

   PrintFormat("STRUCTURAL_GEOMETRY|dir=%s|lookback=%d|atr=%.5f|extreme=%.5f|stop_buffer_atr=%.5f|min_risk_atr=%.5f|target_buffer_ticks=%d|entry=%.5f|stop=%.5f|target=%.5f|p1=%.5f|valid=%s",
               m_setup.dir == SIGNAL_BUY ? "BUY" : "SELL", lookback, atr,
               structural_extreme, InpStructuralStopBufferATR,
               InpStructuralMinRiskATR, InpStructuralTargetBufferTicks,
               m_setup.entry_price,
               m_setup.stop_price, m_setup.target_price, m_setup.P1,
               valid ? "true" : "false");
   return valid;
}

// Measure the setup after its market-derived levels exist. Costs and adverse
// slippage are evaluated per reference volume, so the ratio stays independent
// of the lot subsequently selected by the risk and margin engines.
bool EvaluateSetupRewardRisk(const bool emit_log, const bool enforce_threshold)
{
   m_setup.gross_reward_risk = 0.0;
   m_setup.net_reward_risk = 0.0;
   if ((m_setup.dir != SIGNAL_BUY && m_setup.dir != SIGNAL_SELL) ||
       m_setup.entry_price <= 0.0 || m_setup.stop_price <= 0.0 ||
   ...
   if (tick_size <= 0.0 || min_volume <= 0.0 || max_volume < min_volume)
      return false;

   double reference_volume = MathMin(1.0, max_volume);
   if (reference_volume < min_volume)
      reference_volume = min_volume;

   ENUM_ORDER_TYPE calc_type = (m_setup.dir == SIGNAL_BUY)
                               ? ORDER_TYPE_BUY : ORDER_TYPE_SELL;
   double adverse_entry = m_setup.entry_price;
   double adverse_stop = m_setup.stop_price;
   ...
   if (!OrderCalcProfit(calc_type, _Symbol, reference_volume,
                        adverse_entry, adverse_stop, stop_profit) ||
       !OrderCalcProfit(calc_type, _Symbol, reference_volume,
                        adverse_entry, m_setup.target_price, target_profit) ||
       !MathIsValidNumber(stop_profit) ||
       !MathIsValidNumber(target_profit) ||
       stop_profit >= 0.0 || target_profit <= 0.0)
      return false;

   double gross_loss = -stop_profit;
   double cost = InpEstimatedRoundTurnCostPerLot * reference_volume;
   double net_loss = gross_loss + cost;
   double net_reward = target_profit - cost;
   if (!MathIsValidNumber(cost) || cost < 0.0 ||
       !MathIsValidNumber(net_loss) || net_loss <= 0.0 ||
       !MathIsValidNumber(net_reward) || net_reward <= 0.0)
      return false;

   m_setup.gross_reward_risk = target_profit / gross_loss;
   m_setup.net_reward_risk = net_reward / net_loss;
   bool accepted = (!enforce_threshold || InpMinNetRewardRisk <= 0.0 ||
                    m_setup.net_reward_risk + 1e-9 >= InpMinNetRewardRisk);

   if (emit_log || !accepted)
      PrintFormat("SETUP_REWARD_RISK|mode=%s|entry=%.5f|stop=%.5f|target=%.5f|gross_rr=%.5f|net_rr=%.5f|min_net_rr=%.5f|threshold_enforced=%s|accepted=%s",
                  ExitGeometryModeName(), m_setup.entry_price,
                  m_setup.stop_price, m_setup.target_price,
                  m_setup.gross_reward_risk, m_setup.net_reward_risk,
                  InpMinNetRewardRisk, enforce_threshold ? "true" : "false",
                  accepted ? "true" : "false");
   return accepted;
}
...
bool ValidateInputs()
{
   ...
   if ((int)InpExitGeometryMode < (int)EXIT_GEOMETRY_LEGACY ||
       (int)InpExitGeometryMode > (int)EXIT_GEOMETRY_STRUCTURE)
   {
      Print("VALIDATION ERROR: InpExitGeometryMode is invalid.");
      return false;
   }
   if (InpStructuralStopLookbackBars < 1 ||
       InpStructuralStopLookbackBars > 10000 ||
       !MathIsValidNumber(InpStructuralStopBufferATR) ||
       InpStructuralStopBufferATR < 0.0 ||
       InpStructuralStopBufferATR > 100.0 ||
       !MathIsValidNumber(InpStructuralMinRiskATR) ||
       InpStructuralMinRiskATR <= 0.0 || InpStructuralMinRiskATR > 100.0 ||
       InpStructuralTargetBufferTicks < 0 ||
       InpStructuralTargetBufferTicks > 10000 ||
       !MathIsValidNumber(InpMinNetRewardRisk) ||
       InpMinNetRewardRisk < 0.0 || InpMinNetRewardRisk > 100.0)
   {
      Print("VALIDATION ERROR: Structural exit or net reward/risk parameters are invalid.");
      return false;
   }
   if (InpExitGeometryMode == EXIT_GEOMETRY_STRUCTURE &&
       (InpUseAdaptiveSL || InpUseAdaptiveTP || InpUseSweepBuffer ||
        InpUsePartialTP))
   {
      Print("VALIDATION ERROR: Structural exits cannot be combined with legacy adaptive/sweep/partial exit geometry.");
      return false;
   }
...
// Find the greatest broker-valid volume that is no larger than the
// risk-derived request and whose estimated margin stays inside the configured
// share of free margin. Search discrete volume steps because broker margin can
// be tiered and must not be assumed linear.
bool FindMarginCappedVolume(const ENUM_ORDER_TYPE calc_type,
                            const double requested_volume,
                            const double min_volume,
                            const double step_volume,
                            const double price,
                            const double allowed_margin,
                            double &capped_volume,
                            double &capped_margin,
                            double &requested_margin)
{
   capped_volume = 0.0;
   capped_margin = 0.0;
   requested_margin = 0.0;
   if (requested_volume <= 0.0 || min_volume <= 0.0 ||
       step_volume <= 0.0 || price <= 0.0 || allowed_margin <= 0.0)
      return false;

   bool saw_valid_margin = false;
   ResetLastError();
   if (OrderCalcMargin(calc_type, _Symbol, requested_volume,
                       price, requested_margin) &&
       MathIsValidNumber(requested_margin) && requested_margin > 0.0)
   {
      saw_valid_margin = true;
      if (requested_margin <= allowed_margin)
      {
         capped_volume = requested_volume;
         capped_margin = requested_margin;
         return true;
      }
   }

   int volume_digits = VolumeDigits(step_volume);
   long low_steps = (long)MathCeil(min_volume / step_volume - 1e-9);
   long high_steps = (long)MathFloor(requested_volume / step_volume + 1e-9);
   if (low_steps < 1)
      low_steps = 1;
   if (high_steps < low_steps)
      return saw_valid_margin;

   long best_steps = 0;
   double best_margin = 0.0;
   while (low_steps <= high_steps)
   {
      long mid_steps = low_steps + (high_steps - low_steps) / 2;
      double candidate = NormalizeDouble((double)mid_steps * step_volume,
                                         volume_digits);
      double candidate_margin = 0.0;
      ResetLastError();
      bool margin_ok = (candidate >= min_volume &&
                        candidate <= requested_volume &&
                        OrderCalcMargin(calc_type, _Symbol, candidate,
                                        price, candidate_margin) &&
                        MathIsValidNumber(candidate_margin) &&
                        candidate_margin > 0.0);
      if (margin_ok)
      {
         saw_valid_margin = true;
         if (candidate_margin <= allowed_margin)
         {
      ...
      // An invalid calculation is treated conservatively like an unaffordable
      // candidate, while still allowing the search to try smaller volumes.
   ...
   if (best_steps > 0)
   {
      capped_volume = NormalizeDouble((double)best_steps * step_volume,
                                      volume_digits);
      capped_margin = best_margin;
   }
   return true;
}
...
bool CalculatePositionSize(double entry, double sl, ENUM_ORDER_TYPE order_type, double &out_vol)
{
   ...
   m_last_sizing_actual_loss = 0.0;
   m_last_sizing_actual_risk_pct = 0.0;
   m_last_sizing_required_margin = 0.0;
   m_last_sizing_margin_limit = 0.0;
   m_last_sizing_margin_capped = false;
   double equity = AccountInfoDouble(ACCOUNT_EQUITY);
   ...
   m_last_sizing_margin_limit = allowed_margin;
   if (!MathIsValidNumber(free_margin) || free_margin <= 0.0 ||
       !MathIsValidNumber(allowed_margin) || allowed_margin <= 0.0)
   {
      out_vol = 0.0;
      return RejectSizing(FUNNEL_REJECT_SIZE_MARGIN);
   }

   double requested_volume = out_vol;
   double capped_volume = 0.0;
   double required_margin = 0.0;
   double requested_margin = 0.0;
   if (!FindMarginCappedVolume(calc_type, requested_volume, min_vol,
                               step_vol, adverse_entry, allowed_margin,
                               capped_volume, required_margin,
                               requested_margin))
   {
      if (InpVerboseLog)
         PrintFormat("GUARD REJECT: OrderCalcMargin failed for every tested volume. Error: %d",
                     GetLastError());
      out_vol = 0.0;
      return RejectSizing(FUNNEL_REJECT_SIZE_MARGIN);
   }
   ...
   {
      PrintFormat("SIZING_REJECT_MARGIN_MIN|requested_volume=%.8f|requested_margin=%.2f|margin_limit=%.2f|min_lot=%.8f",
                  requested_volume, requested_margin, allowed_margin, min_vol);
      out_vol = 0.0;
      return RejectSizing(FUNNEL_REJECT_SIZE_MARGIN);
   }

   m_last_sizing_margin_capped =
      (capped_volume < requested_volume - step_vol * 0.5);
   out_vol = capped_volume;
   m_last_sizing_required_margin = required_margin;

   // Recalculate the exact final volume and reject even a rounding-cent
   ...
   m_last_sizing_actual_loss = exact_worst_loss;
   m_last_sizing_actual_risk_pct = 100.0 * exact_worst_loss / equity;
   ...
   if (!MathIsValidNumber(m_last_sizing_actual_risk_pct) ||
       !MathIsValidNumber(required_margin) || required_margin <= 0.0 ||
       required_margin > allowed_margin)
   {
      if (InpVerboseLog)
         PrintFormat("GUARD REJECT: Final volume margin %.2f exceeds limit %.2f.",
                     required_margin, allowed_margin);
      out_vol = 0.0;
      return RejectSizing(FUNNEL_REJECT_SIZE_MARGIN);
   }

   if (m_last_sizing_margin_capped)
      RecordFunnel(FUNNEL_SIZE_MARGIN_CAPPED);
   PrintFormat("SIZING_RESULT|requested_risk_pct=%.5f|risk_budget=%.2f|requested_volume=%.8f|requested_margin=%.2f|actual_volume=%.8f|actual_worst_loss=%.2f|actual_risk_pct=%.5f|actual_margin=%.2f|margin_limit=%.2f|margin_capped=%s",
               InpRiskPercent, risk_money, requested_volume, requested_margin,
               out_vol, exact_worst_loss, m_last_sizing_actual_risk_pct,
               required_margin, allowed_margin,
               m_last_sizing_margin_capped ? "true" : "false");
                          ...
                          double immutable_original_stop)
{
   ...
   // Restore from immutable entry/TP geometry. Structural mode targets the
   // known P1 level (less a deterministic tick buffer), so P0/range remain
   // exactly reconstructible without terminal-only state. The live SL may
   // already have moved and therefore never defines the original range.
   double tick_size = SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_SIZE);
   if (tick_size <= 0.0 || !MathIsValidNumber(tick_size))
      return false;

   double restored_p1 = target;
   double ratio_distance = InpTargetRatio - InpEntryRatio;
   if (InpExitGeometryMode == EXIT_GEOMETRY_STRUCTURE)
   {
      double target_buffer = (double)InpStructuralTargetBufferTicks * tick_size;
      restored_p1 = (dir == SIGNAL_BUY)
                    ? target + target_buffer : target - target_buffer;
      ratio_distance = 1.0 - InpEntryRatio;
   }
   if (ratio_distance <= 0.0)
      return false;

   double restored_range = (dir == SIGNAL_BUY)
                           ? (restored_p1 - entry) / ratio_distance
                           : (entry - restored_p1) / ratio_distance;
   ...
   if (dir == SIGNAL_BUY)
   {
      ...
      m_setup.visual_target_price = (InpExitGeometryMode == EXIT_GEOMETRY_STRUCTURE)
                                    ? NormalizePriceDirectional(m_setup.P1, 0)
                                    : NormalizePriceDirectional(m_setup.P0 + InpVisualTargetRatio * restored_range, 0);
   }
   else
   {
      m_setup.P0 = restored_p0;
   ...
   if (!EvaluateSetupRewardRisk(false, false))
      return false;
...
void SyncState()
{
   ...
   if (snapshot.managed_positions == 1)
   {
      ...
      m_setup.position_ticket = snapshot.position_ticket;
      if (!EvaluateSetupRewardRisk(false, false) && InpVerboseLog)
         Print("WARNING: [RSIFibEA] Position reward/risk diagnostics could not be refreshed from the actual fill.");
...
void UpdateDashboard()
{
   ...
   Comment(StringFormat("RSI Fib EA v4.52 Research | DEMO GUARD: %s\n"
                        "State: %s | Direction: %s | Protection: %s\n"
                        "Spread: %.1f pts | Order: %llu | Position: %llu\n"
                        "P0 %.5f | P1 %.5f | Entry %.5f | SL %.5f | TP %.5f | TP1 %.5f\n"
                        "Exit: %s | gross RR %.3f | net RR %.3f\n"
                        "Sizing: requested %.3f%% | actual %.3f%% | margin cap: %s\n"
                        "Filters: %s\nStatus: %s",
                        ...
                        m_setup.entry_price, m_setup.stop_price, m_setup.target_price,
                        m_setup.partial_tp_price, ExitGeometryModeName(),
                        m_setup.gross_reward_risk, m_setup.net_reward_risk,
                        InpRiskPercent,
                        m_last_sizing_actual_risk_pct,
                        m_last_sizing_margin_capped ? "YES" : "NO",
                        filters, status));
}
...
void DrawSetupObjects()
{
   ...
   string stop_text = (InpExitGeometryMode == EXIT_GEOMETRY_STRUCTURE)
                      ? "Stop (structure + ATR)"
                      : StringFormat("Stop (%.2f)", InpStopRatio);
   CreateHLine(m_obj_prefix + "Stop", m_setup.stop_price, col_stop, STYLE_SOLID, 2, stop_text);
   ...
   string target_text = (InpExitGeometryMode == EXIT_GEOMETRY_STRUCTURE)
                        ? StringFormat("Target P1 (net %.2fR)", m_setup.net_reward_risk)
                        : StringFormat("Target (%.2f)", InpTargetRatio);
   string visual_text = (InpExitGeometryMode == EXIT_GEOMETRY_STRUCTURE)
                        ? "P1 liquidity"
                        : StringFormat("Visual Target (%.2f)", InpVisualTargetRatio);
   CreateHLine(m_obj_prefix + "Target", m_setup.target_price, col_target, STYLE_SOLID, 2, target_text);
   CreateHLine(m_obj_prefix + "VisualTarget", m_setup.visual_target_price, col_vtarget, STYLE_DASH, 1, visual_text);
}
