...
#property description "RSI Custom Fibonacci Limit Expert Advisor for MT5 (Demo Only)"
#property strict
...
void ProcessStateIdle()
{
   ...
   if (!CheckRiskGuards() || !CheckSpread() || !CheckSession())
   {
      if (InpVerboseLog)
...
void ProcessStatePendingOrder(bool is_new_bar)
{
   ...
   if (is_new_bar)
   {
      ...
      if (!CheckRiskGuards())
      {
         Print("INFO: [RSIFibEA] Daily risk guard threshold breached. Cancelling pending order.");
         CancelPendingOrder();
         return;
      }
   }
...
void ExecutePendingOrder(ENUM_ORDER_TYPE order_type, double vol)
{
   ...
   if ((expiration_modes & (long)SYMBOL_EXPIRATION_SPECIFIED) != 0 && timeframe_seconds > 0)
   {
      lifetime_type = ORDER_TIME_SPECIFIED;
      broker_expiration = TimeCurrent() + (datetime)((long)timeframe_seconds * InpPendingOrderBars);
   }
   else if ((expiration_modes & (long)SYMBOL_EXPIRATION_GTC) != 0)
   {
      lifetime_type = ORDER_TIME_GTC;
   }
   else if ((expiration_modes & (long)SYMBOL_EXPIRATION_DAY) != 0)
   {
      lifetime_type = ORDER_TIME_DAY;
   }
   else if ((expiration_modes & (long)SYMBOL_EXPIRATION_SPECIFIED_DAY) != 0 && timeframe_seconds > 0)
   {
   ...
   {
      Print("ERROR: [RSIFibEA] Symbol exposes no supported pending-order expiration mode. Setup aborted.");
      ResetSetupToIdle();
      return;
   }
...
bool ValidateInputs()
{
   ...
   {
      Print("VALIDATION ERROR: InpRiskPercent must be between 0.01 and 10.0.");
      return false;
   }
...
bool CalculatePositionSize(double entry, double sl, ENUM_ORDER_TYPE order_type, double &out_vol)
{
   ...
   if (!OrderCalcProfit(calc_type, _Symbol, 1.0, entry, sl, profit_1lot))
   {
      if (InpVerboseLog)
         PrintFormat("WARNING: OrderCalcProfit failed for 1.0 lot. Error: %d", GetLastError());
      return false;
   }

   if (!MathIsValidNumber(profit_1lot) || profit_1lot >= 0.0)
   {
      if (InpVerboseLog)
         PrintFormat("WARNING: OrderCalcProfit did not return a valid loss: %.2f", profit_1lot);
      return false;
   }

   double loss_1lot = -profit_1lot;

   double raw_vol = risk_money / loss_1lot;
   out_vol = NormalizeVolume(raw_vol);

   if (out_vol < SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN))
      return false;
